...
> = {
  ...
  residence: {
    labelKey: "info.typeResidence",
    color: "bg-amber-900 text-amber-200",
  },
  government: {
    labelKey: "info.typeGovernment",
...
function CuratedPanel({ structure }: { structure: Structure }) {
  ...
  const desc = lang === "zh" ? structure.descriptionChinese : structure.description
  ...
  return (
    <>
      ...
      {desc && (
        <p className="text-stone-300 text-sm leading-relaxed mb-4">{desc}</p>
      )}
      {structure.architect && (
        <div className="mb-2">
          <span className="text-stone-500 text-xs uppercase tracking-wide">
            {t("info.fieldArchitect")}
          </span>
          <p className="text-stone-300 text-sm">
            {lang === "zh" ? (structure.architectChinese ?? structure.architect) : structure.architect}
          </p>
        </div>
      )}
      {structure.architecturalStyle && (
        <div className="mb-2">
          ...
          <p className="text-stone-300 text-sm">
            {lang === "zh" ? (structure.architecturalStyleChinese ?? structure.architecturalStyle) : structure.architecturalStyle}
          </p>
        </div>
      )}
      {structure.keyFeatures && structure.keyFeatures.length > 0 && (
        <div className="mb-2">
          <span className="text-stone-500 text-xs uppercase tracking-wide">
            {t("info.fieldFeatures")}
          </span>
          <ul className="mt-1 space-y-1">
            {(lang === "zh" ? (structure.keyFeaturesChinese ?? structure.keyFeatures) : structure.keyFeatures).map((f, i) => (
      ...
      {structure.materials && structure.materials.length > 0 && (
        <div className="mb-2">
          <span className="text-stone-500 text-xs uppercase tracking-wide">
            {t("info.fieldMaterials")}
          </span>
          <p className="text-stone-300 text-sm">
            {(lang === "zh" ? (structure.materialsChinese ?? structure.materials) : structure.materials).join(", ")}
          </p>
        </div>
      )}
      ...
      {structure.imageCredit && (
        <p className="text-stone-600 text-xs mt-4">
          {t("info.imageCredit")}: {structure.imageCredit}
        </p>
      )}
    </>
  )
      ...
      >
        ...
        <div className="flex items-start justify-between px-4 py-3 border-b border-stone-800 shrink-0">
          <div className="flex-1 min-w-0">
            <h2
              className={`text-stone-100 font-semibold text-base leading-tight truncate ${lang === "zh" ? "font-chinese" : ""}`}
            >
              {displayName}
            </h2>
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              ...
              <span className="text-stone-500 text-xs">
                {lang === "zh" ? (structure.dynastyChinese ?? structure.dynasty) : structure.dynasty} · {yearLabel}
              </span>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {structure.source === "curated" ||
          structure.source === "ai-identified" ? (
